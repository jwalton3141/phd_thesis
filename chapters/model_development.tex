...
$\mathcal{U}(-\eta/2, \eta/2)$. The directional update of agent $i$ can then be realised by the draw
\begin{equation}
...
\end{equation}
At time $t$, a flock not moving cohesively, with individual members directed randomly, has
polarisation $v_{a,t}\approx0$. Conversely, a highly polarised flock in which all members are moving in
the same direction at time $t$ has an alignment $v_{a,t}\approx1$.
...
\begin{figure}[tb]
    ...
    \caption{Visualising the relationship between interaction strength $\omega_{ij,t}$ and
        ...
        represent: the Vicsek interaction (\cref{eq:vicsek_interaction}), the power-law
...
\begin{equation}
	...
	\end{cases}
\end{equation}
The interaction of the Vicsek model is parameterised by the radius $r$. Here the
...
$i$'s behaviour. It follows from this that the further away a neighbour is from the
position of agent $i$, the less influence it has on agent $i$'s movement.

This interaction rule is continuous for $d_{ij,t} > 0$, but still exhibits a discontinuity
at $d_{ij,t}=0$. Although this may present a problem theoretically, in practice we do not
...
neighbour $j$ exerts over agent $i$ is inversely related to their distance apart; the
...
Here the parameter $\sigma_X$ sets the length-scale over which the 

\subsection{Topological models}

Much interest in topological interaction rules was generated by \cite{ballerini08}. In
this model agents interact wither their $k\in\mathbb{N}$ nearest neighbours. Here we look
to relax the constraint in which agents interact with some fixed integer number of closest
neighbours. Instead, we allow agents to respond to the movements of partial neighbours and 
