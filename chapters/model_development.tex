...
alternative. Such rules ensure the model is more robust to small perturbations in
distances, but without the penalty of adding extra model complexity.
...
$\theta_{i, t+1}$. Updating $\theta{i,t}$ to $\theta_{i, t+1}$ models how agents update
...
distances.  More formally, this discontinuity can be considered by the value of
...
Developed from models of ferromagnetism, the uniformly distributed noise implemented in
...
$\nu$ degrees of freedom. The directional update of agent $i$ is then expressed as:
\begin{equation*}
...
with location $\mu$ and scale $\sigma_Y$ recovers a normal distribution with mean and
...
\begin{figure}[tb]
    ...
    \caption{A trajectory plot representing the movements of $N=45$ agents moving in an
    unrestricted domain. Agents are initially positioned at the locations represented by
    the green markers. The model is simulated for $200$ time steps. The red markers
    represent the positions of agents at the end of a simulation. Blue lines represent
    the trajectories of motion of individual agents.}
