...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \For{i}{1}{n}
      ...
      \If{$u \leq \alpha(\bm{\theta}^\star \given \bm{\theta}^{(i-1)})$}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \For{i}{1}{n}
      \State \Comment{Mass matrix $M$}
      \State Draw momentum $p^{(i-1)} \sim \mathcal{N}_d(0, M)$
      \State $p^\star \leftarrow p^{(i-1)}$
      \State $q^\star \leftarrow q^{(i-1)}$
      \State \Comment{Simulate Hamiltonian with leapfrog method for $L$ steps and step
      size $\epsilon$.}
      \For{j}{1}{L}
      ...
      \State Construct acceptance probability $\alpha(q^\star, p^\star \given
        ...
        \end{equation*}
      \State Draw $u \sim$ Uniform$(0, 1)$
      \If{$u \leq \alpha(q^\star, p^\star \given q^{(i-1)}, p^{(i-1)})$}
        \State \Comment{Accept proposal}
