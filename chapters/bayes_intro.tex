...
\begin{figure}[tpb]
  ...
  \caption{Example Markov chains produced by a sampler.
      \subref{fig:good_mixing} A chain which looks to be mixing well, and
      ...
      \subref{fig:poor_mixing} A poorly mixing chain. Realisations
...
(or close to independent) groups. It has been observed that this can be very
difficult to achieve for time-series data or data with a spatial component.
Since models of collective behaviour have both a time \emph{and} spatial
...
information criterion as it is easy to compute from MCMC samples. The deviance
information criterion penalises model complexity and so is designed to provide
a measure which trades-off between model-fit and model-complexity. DIC makes
use of the deviance, defined as:
\begin{equation*}
    D(\theta) = -2 \log (x \given \theta).
\end{equation*}

With this DIC can then be defined as:
\begin{equation*}
    \text{DIC} = \overline{D(\theta)} + p_D,
\end{equation*}
where $\bar{D(\theta)}$ is the mean of the deviance and $p_D$ is the
effective number of parameters. The effective number of parameters is defined
as $p_D = \overline{D(\theta)} - D(\overline{\theta})$.
