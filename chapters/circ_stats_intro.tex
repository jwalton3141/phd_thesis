...
%NOTES: consider adding section on circular distributions. Show limiting result of von Mise distributio
...
\centering
	...
	\end{subfigure}
	\caption{Comparing \subref{subfig:radian_axes}: radian and \subref{subfig:degree_axes}: degree measuri
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Using histograms to visualise \subref{subfig:unif_angle_hist}: 100 samples from $U(-\pi, \pi)
$ and \subref{subfig:norm_angle_hist}: 10\thinspace 000 draws from $N(0, 1)$.}
...
\begin{figure}
	...
	\end{subfigure}
	\caption{Using polar histograms to visualise the dataset generated from \subref{subfig:unif_angle_rose
}: $U(-\pi,\pi)$ and \subref{subfig:norm_angle_rose}: $N(0, 1)$.}
...
et consists of 10\thinspace 000 draws from a normal $N(0, 1)$ distribution.

In \cref{fig:angle_hist} we visualise the two datasets using traditional histogram plots. From this fig
...
Before introducing the circular mean it is first necessary to introduce the $\atantwo$ function. The $\
...
The $\atantwo$ function, however, does not have these shortcomings. The function is constructed to be q
