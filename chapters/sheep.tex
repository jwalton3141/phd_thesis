...
sequence $1$ are shown in \cref{fig:posterior_seq1}. Our prior beliefs about
...
have not yet assessed their adequacy or goodness-of-fit. To do so we
shall inspect residual plots and perform posterior predictive checks.
...
Although an informal assessment of the standardised residuals of the model fits
appears to show that all the models perform similarly, it can be useful to
quantify their performance more formally with an information criteria.
...
\bottomrule
\end{tabular}
\caption{Tabulating AIC and the corresponding Akaike weights of each model fit
to sequence $1$. }
\label{tab:aic_seq1}
\end{table}
...
\begin{figure}[tbp]
    ...
    \caption{Posterior predictive checks for sequence $1$.}
