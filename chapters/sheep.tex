...
\begin{figure}[tbp]
    ...
    \caption{Alignment as a function of time for forward simulated flocks,
    inferred from sequence $3$.}
    \label{fig:checks_seq3}
\end{figure}

\subsection{Conclusions}

We have seen that we are able to fit a number of competing agent-based models
to real flocking events, working in a Bayesian framework. Model performance can
be assessed using a number of methods. Residual plots can be visualised to
provide an informal method to compare model fit and data. However, a more
quantitative comparison of model fit can be realised with information criteria.

Our posterior beliefs consistently showed that only weak interaction parameters
are necessary to maintain flocking events which are already highly-polarised.
