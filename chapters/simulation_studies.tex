...
Introducing this extra level of structure into our model needs to be accounted for in the
...
In \cref{ssec:cont_models} we used Stan to perform parameter inference on data simulated
from \emph{global} continuous models. Similarly, here we shall use Stan to perform
parameter inference on data simulated from our \emph{hierarchical} continuous models.

\subsubsection{Power-law weighted}

\cref{fig:power_hier_sim_study} shows data simulated from the hierarchical power-law
...
$m_Y=0.04$ and variance $v_Y=0.001$. These represent the hyperparameters of our model.

\begin{figure}[tbp]
  \includegraphics{power_hier_sim.pdf}
  \caption{Forward simulation of the hierarchical power-law weighted model. The movements
  of $N=45$ agents are simulated for $T=200$ time steps. The interaction parameters
  $\alpha_i$ are drawn from a gamma distribution with mean $m_{\alpha}=1.2$ and variance
  $v_{\alpha}=0.02$. Similarly, the noise-scale parameters $\sigma_{Y_i}$ are realisations
  from a gamma distribution with mean $m_Y=0.04$ and variance $v_Y=0.001$.}
  \label{fig:power_hier_sim_study}
\end{figure}
...
\cref{fig:power_hier_study_hists}. Each coloured histogram overlain represents the
...
line), their sample mean was $\overline{\alpha}\approx1.18$ (vertical black line). In all
cases we see that the hyperparameters are well-captured by our posterior beliefs, and we
conclude that they can be accurately inferred from simulated data.

Posterior beliefs about the $N$ interaction and $N$ noise-scale parameters are summarised
in \cref{fig:power_hier_study_sum}. Here, each boxplot represents a posterior density. As
is convention, box boundaries represent the $25$th and $75$th percentiles of the
posterior. Whiskers represent the median $\pm1.5\times\text{IQR}$. See that in all but one
case ($\alpha_6$), our posterior beliefs capture the true parameter values. We observe
that the $\sigma_{Y_i}$'s are generally well represented by the posterior median. It
appears that smaller noise-scale parameters can be captured with less uncertainty than
larger noise-scale parameters. Although forty-four of the forty-five interaction
parameters are captured by our posterior beliefs, the interaction parameters are not
recovered with as much accuracy of the noise-scale parameters. However, we still see that
our posterior beliefs well represent the true parameter values. 

Although not included here for brevity, tabulating split-$\widehat{R}$ for every
parameter reveals all chains converged with split-$\widehat{R}=1.00$. Computing
the effective sample size for each parameter showed that the \emph{worst} performing
chain still realised approximately $10,000$ effective samples from the posterior.

Considering all the output generated by our Stan model, we have demonstrated that we can
accurately recover the hyperparameters and parameters of data simulated from a hierarchical
power-law weighted model.
...
\begin{figure}[tbp]
  ...
  \caption{Histogram plots representing our posterior beliefs about the hyperparameters of
      the hierarchical power-law weighted model. The vertical green lines represent the
      true values of the hyperparameters---the \emph{population} means and variances of the
      parameters. The vertical black lines represent the \emph{sample} means and variances
      of the parameters. Our hyperpriors are overlain in red, and appear flat in
      comparison to our posteriors.}
  \label{fig:power_hier_study_hists}
\end{figure}
\begin{figure}[tbp]
  \includegraphics{power_hier_summary.pdf}
  \caption{Boxplots summarising our posterior beliefs about the interaction and
  noise-scale parameters for all agents. Each box represents the posterior median along
  with the upper and lower quartiles of these distributions. As is graphical convention,
  the whiskers of the boxes represent the median $\pm 1.5\times\text{IQR}$.}
  \label{fig:power_hier_study_sum}
\end{figure}
...
and variance $\text{Var}(\sigma_X)\approx1.1$.

We use the Stan modelling language to codify \cref{eq:hierarchical_likelihood} and express
our vague prior beliefs about the hyperparameters of the model.

\begin{figure}[tbp]
  \includegraphics{gauss_hier_sim.pdf}
  \caption{Data generated by a simulation of the hierarchical Gaussian weighted model.
  The interaction parameters $\sigma_{X_i}$ are distributed according to a gamma
  distribution with mean $m_X=25$ and variance $v_X=1$. Similarly, the noise-scale
  parameters are drawn from a gamma distribution with mean $m_Y=0.04$ and variance
  $v_Y=0.001$.} 
\end{figure}
\begin{figure}[tbp]
